...
| `GOOGLE_MAPS_JS_API_KEY` | to see the interactive map or the export page's static maps render | Deliberately a **separate** key from `GOOGLE_MAPS_API_KEY` — this one is exposed to the browser. Needs **both** Maps JavaScript API (interactive map, ticket #4) **and** Maps Static API (per-day export maps, ticket #8) allowed on the key, plus an HTTP-referrer restriction set in Cloud Console before it's safe to use anywhere but local dev. If unset, the map area shows "Map isn't configured on this deployment yet." instead of a broken blank space; if set but missing an API's authorization, that specific map degrades to the same text instead of a broken image — see #8's live-verification note below |
...
- **#8's Static Maps API**: live-tested and the deployed `GOOGLE_MAPS_JS_API_KEY` returned `403` ("This API key is not authorized to use this service or API") — same category of issue as the Weather key restriction above, just on a different key/API pair. `static/export.js` was hardened in response: a failed static-map image load (`<img onerror>`) now swaps to the same "map isn't configured" text instead of leaving a broken-image icon in a printed page, so this degrades gracefully rather than looking broken — but add Maps Static API to the key's restrictions in Cloud Console to see real per-day route maps on the export page.
