...
- `agent/llm.py`'s Gemini integration uses the `google-genai` SDK. **Live-verified 2026-07-24**: `gemini-2.0-flash` was in fact retired (confirmed via a real key: `404 NOT_FOUND`, "no longer available", despite still appearing in `models.list()`) — swapped to `gemini-3.1-flash-lite` (maintainer's preference for the 3.1 series), confirmed working with a real key
...
- `agent/tools.py`'s `get_photo_bytes` (Places Photos media endpoint) and `agent/llm.py`'s `generate_cover_image` (via `google-genai`'s `response_modalities=["TEXT", "IMAGE"]`) were both new in #7. **Live-verified 2026-07-24**: same story as text generation — `gemini-2.0-flash-preview-image-generation` is retired; swapped to `gemini-3.1-flash-image`, confirmed to return real image bytes (`part.as_image()`) with a real key. If image generation ever fails or returns no image, `save_completed_trip` in `trips.py` silently falls back to the first real attraction photo already fetched for that trip, so a saved trip should never end up with a missing cover as long as at least one attraction photo fetch succeeds.
- `weather.googleapis.com` returned `403 PERMISSION_DENIED` / `API_KEY_SERVICE_BLOCKED` when live-tested — the Weather API is enabled on the GCP project but wasn't in this specific key's API-restrictions allowlist in Cloud Console. Non-fatal (weather is reference-only and already degrades gracefully — see the Directions-failure handling in `agent/graph.py`'s `act()`), but add Weather API to the key's restrictions to actually see forecasts
