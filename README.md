...
| `GOOGLE_MAPS_JS_API_KEY` | to see the interactive map render | Deliberately a **separate** key from `GOOGLE_MAPS_API_KEY` — this one is exposed to the browser, so it needs Maps JavaScript API enabled and an HTTP-referrer restriction set in Cloud Console before it's safe to use anywhere but local dev. If unset, the map area shows "Map isn't configured on this deployment yet." instead of a broken blank space |
| `GOOGLE_MAPS_STATIC_API_KEY` | to see the export page's per-day static maps render | **Live-corrected 2026-07-24**: originally this shared `GOOGLE_MAPS_JS_API_KEY`, on the theory that one browser-exposed key could cover both Maps JavaScript API and Maps Static API. That failed live — a key restricted to just Maps JavaScript API 403'd on Static Maps requests — so this is now its own env var, letting each key be restricted to exactly the one API it needs. Same browser-exposure/referrer-restriction rules as `GOOGLE_MAPS_JS_API_KEY` apply. If unset, or if the request fails for any reason, each day's map degrades to the same "map isn't configured" text instead of a broken image (`static/export.js`'s `<img onerror>` handler) |
...
- The map (`static/app.js`) was verified with a placeholder key pre-deploy: the day tabs, marker/polyline plumbing, and script loading all work correctly. The first real, deployed `GOOGLE_MAPS_JS_API_KEY` still failed live in-browser (Google's own error overlay) even though a plain `curl` of the `/maps/api/js` bootstrap URL returned 200 either way — the actual key/referrer check happens inside the script's runtime once it's initializing a real map in a real browser, which `curl` can't simulate. Swapped to a second key restricted to just Maps JavaScript API, confirmed working. Note `google.maps.Marker` is deprecated upstream in favor of `AdvancedMarkerElement`; left as-is since migrating requires a Map ID from Cloud Console and `Marker` remains fully supported with no discontinuation date announced.
- **#8's Static Maps API**: live-tested and the first deployed key returned `403` ("This API key is not authorized to use this service or API") — same category of issue as the Weather key restriction above, just on a different key/API pair. `static/export.js` was hardened in response: a failed static-map image load (`<img onerror>`) now swaps to the same "map isn't configured" text instead of leaving a broken-image icon in a printed page, so this degrades gracefully rather than looking broken. Root cause turned out to be architectural, not just a missing checkbox: one browser-exposed key restricted to Maps JavaScript API can't also be authorized for Maps Static API without widening that key's blast radius — so `GOOGLE_MAPS_STATIC_API_KEY` was split into its own env var (see the table above), each key restricted to exactly the one API it needs. Confirmed working with a dedicated Static Maps key.
- **Origin round trip + clock schedule + descriptions**, live-verified against the deployed app with real credentials (a real Taipei/"Taipei 101" run): the Directions round trip, the 08:00-anchored per-stop arrival/departure times (correctly offset by real driving minutes from the origin, e.g. `08:07` for the first stop rather than a flat `08:00`), and Gemini-generated Traditional Chinese descriptions for every stop all worked end-to-end with no code changes needed.
...
**`GOOGLE_MAPS_JS_API_KEY` and `GOOGLE_MAPS_STATIC_API_KEY` must both be in `--set-env-vars`** — easy to miss since they look like they belong with the two Secret Manager keys, but `/api/config` reads them straight from the environment; omitting either silently serves an empty key and that specific map never renders on the deployed site.

Prefer Secret Manager (`--set-secrets`) over `--set-env-vars` for `GOOGLE_MAPS_API_KEY` and `GEMINI_API_KEY` — they're real credentials. `GOOGLE_MAPS_JS_API_KEY` and `GOOGLE_MAPS_STATIC_API_KEY` are the exceptions: both are meant to be browser-visible, so plain env vars are fine, but restrict each by HTTP referrer to the Hosting domain in Cloud Console before deploying, and by API restriction to exactly the one API each needs (Maps JavaScript API / Maps Static API respectively) — an unrestricted key in page source can be lifted and used to run up usage on any API it's allowed to call, and live-testing this app found that one key covering both APIs is genuinely more fragile to get right than two narrowly-scoped keys.
